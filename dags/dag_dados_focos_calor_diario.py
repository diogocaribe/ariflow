...
# Definição do DAG
dag = DAG(
    "inpe_queimadas_bahia_diario",
    default_args=default_args,
    description="Download e carga de dados de queimadas da Bahia - INPE",
    schedule_interval="@daily",  # Executa mensalmente
    catchup=True,
    max_active_runs=5,
...
def data_execucao_dag(**context):
    ds_nodash = context["ds_nodash"]  # string '20250915'
    return ds_nodash
...
def download_and_extract_data(**context):
    ...
    """
    day = data_execucao_dag(**context)

    filename = f"focos_diario_br_{day}.csv"
    url = f"{BASE_URL}{filename}"
    file_path = os.path.join(DATA_DIR, filename)

    try:
        logging.info(f"Baixando arquivo de {day}: {url}")
        ...
        logging.warning(f"Erro ao baixar arquivo para o ano {day}: {e}")
...
def load_data_to_postgres(**context):
    ...
    file_name = f"focos_diario_br_{day}.csv"
    ...
    try:
        ...
        df.rename(columns={"data_hora_gmt": "data"}, inplace=True)
